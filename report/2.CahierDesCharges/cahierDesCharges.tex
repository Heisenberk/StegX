...
$n$ pour la taille en octets du fichier à cacher et $m$ pour la taille en octets du fichier hôte. 
...
La méthode utilisée a pour but de cacher des données à des endroits différents
à chaque mot de passe. 
...
minime de couleur induis par la modification. 

\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme LSB sans Mot De Passe}
\begin{algorithmic}
...
\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme LSB avec Mot De Passe}
\begin{algorithmic}
\STATE ECRIRE ALGO
\end{algorithmic}
\end{algorithm}
...
La complexité de cet algorithme se calcule de la manière suivante : 
la première boucle TantQue a une complexité de $O(m-n)$, la deuxième $O(n)$. 
\newline Donc la complexité de l'algorithme LSB est de $O(m-n)+O(n)=O(m-n+n)=O(m)$
...
\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme EOF sans Mot De Passe}
\begin{algorithmic}
\STATE \footnotesize Ouverture fichier à cacher (LECTURE) $\rightarrow$ $f1$
\STATE Ouverture fichier hôte (LECTURE) $\rightarrow$ $f2$
\STATE Ouverture fichier final (ECRITURE) $\rightarrow$ $f3$
...
\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme EOF avec Mot De Passe}
\begin{algorithmic}
\STATE ECRIRE ALGO
\end{algorithmic}
\end{algorithm}
...
Pour cet algorithme, la première boucle TantQue a une complexité de $O(m)$, 
...
\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme MetaData sans Mot De Passe}
\begin{algorithmic}
\STATE \footnotesize Ouverture fichier à cacher (LECTURE) $\rightarrow$ $f1$
\STATE Ouverture fichier hôte (LECTURE) $\rightarrow$ $f2$
\STATE Ouverture fichier final (ECRITURE) $\rightarrow$ $f3$
...
\begin{minipage}{.5\textwidth}
\begin{algorithm}[H]
\caption{Algorithme MetaData avec Mot De Passe}
\begin{algorithmic}
\STATE ECRIRE ALGO
\end{algorithmic}
\end{algorithm}
