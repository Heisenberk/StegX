...
\textit{hidden\_length}, \textit{algo}, \textit{hidden\_name} et 
\textit{method} seront initialisés. 
\newline
...
\textit{stegx\_suggest\_algo} et \text{stegx\_choose\_algo} du sous-module 
...
Le sous-module \textit{Détection de l'algorithme de stéganographie} va
compléter les champs des structures \textit{info\_s} 
...
correctement les données cachées. Pour cela, il va lire le fichier à analyser 
et déceler l'algorithme. 
...
Cette fonction va, à l'aide de la fonction \textit{fill\_host\_file\_type}, 
initialisé le champ-union \textit{infos.host.file\_type}. Puis, la 
...
Elle va ouvrir le fichier \textit{infos.res} à partir du paramètre
...
%Si l'utilisateur veut dissimuler des données
\newline
\underline{Entrées :} 
\begin{itemize}
\item \textit{*infos :} structure représentant toutes les informations pour 
