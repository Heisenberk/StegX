...
D'après l'organigramme créé lors du cahier des charges, ce module récupère 
les chemins du fichier hôte et du fichier à cacher (pour la dissimulation) ; 
...
Enfin, après la vérification, les fichiers vérifiés seront envoyés aux modules 
...
\begin{lstlisting}[language=c]
int stegx_check_compatibility_insert (stegx_choices_s* choices, 
	info_s* infos);
\end{lstlisting}

Cette fonction récupère le champ \textit{host\_path} et \textit{insert\_info.hidden\_path}
de \textit{choices} pour connaître le nom du fichier hôte et le nom du fichier 
à cacher. Elle va ensuite interagir avec le système de gestion de fichiers 
pour les ouvrir. 
...
\begin{lstlisting}[language=c]
int stegx_check_compatibility_extract (stegx_choices_s* choices, 
	info_s* infos);
\end{lstlisting}

Cette fonction récupère le champ \textit{host\_path} 
de \textit{choices} pour connaître le nom du fichier à analyser. 
Elle va ensuite interagir avec le système de gestion de fichiers 
pour les ouvrir. 
Elle vérifie que le fichier existe et qu'il n'est pas vide. Puis, la 
...
fichier hôte, cette fonction va remplir le champ \textit{host.file\_infos} 
de \textit{infos}. En effet, \textit{file\_infos} est une union et est 
...
\begin{lstlisting}[language=c]
int stegx_suggest_algo (info_s* infos, stegx_choices_s* choices);
\end{lstlisting}
...
\begin{lstlisting}[language=c]
int stegx_choose_algo (info_s* infos, stegx_choices_s* choices); 
\end{lstlisting}
...
\begin{lstlisting}[language=c]
int read_signature (info_s* infos, stegx_choices_s* choices); 
\end{lstlisting}
...
\begin{lstlisting}[language=c]
int stegx_detect_algo (info_s* infos, stegx_choices_s* choices); 
\end{lstlisting}
...
\begin{lstlisting}[language=c]
int insert (info_s* infos, stegx_choices_s* choices); 
\end{lstlisting}
...
\begin{lstlisting}[language=c]
int extract (info_s* infos, stegx_choices_s* choices); 
\end{lstlisting}
...
\section{Représentation des informations circulantes entre les modules}
