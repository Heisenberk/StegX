...
int insert_eof(info_s* infos){
	...
	if((infos->host.type==BMP_COMPRESSED)||(infos->host.type==BMP_UNCOMPRESSED)
		||(infos->host.type==PNG)||(infos->host.type==WAV_NO_PCM)||((infos->host.type==WAV_PCM))){
		if((infos->host.type==BMP_COMPRESSED)||(infos->host.type==BMP_UNCOMPRESSED)){
	...
	// a revoir
	if((infos->host.type==MP3)||(infos->host.type==AVI_COMPRESSED)||(infos->host.type==AVI_UNCOMPRESSED)){
