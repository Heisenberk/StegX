...
int insert_junk_chunk(info_s * infos)
{
    ...
    if (infos->hidden_length > LENGTH_FILE_MAX)
        return data_xor_write_file(infos->hidden, infos->res, infos->passwd) ? perror("JUNK_CHUNK: Can't write XORed hidden data"), 1 : 0;
    ...
    return data_scramble_write(infos->hidden, infos->res, infos->passwd, infos->hidden_length, infos->mode) ?
        perror("JUNK_CHUNK: Can't write scrambled hidden data"), 1 : 0;
...
int extract_junk_chunk(info_s * infos)
{
     ...
     * suite pseudo aléatoire générée avec le mot de passe. */
    if (infos->hidden_length > LENGTH_FILE_MAX)
        return data_xor_write_file(infos->host.host, infos->res, infos->passwd) ? perror("JUNK_CHUNK: Can't write deXORed hidden data"), 1 : 0;
    /* Sinon on utilise la méthode de protection des données du mélange
     * des octets. */
