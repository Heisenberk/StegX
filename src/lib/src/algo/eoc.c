	...
	do{
		...
		else 
		{
			...
			if (cpt_video_tag==infos->host.file_info.flv.nb_video_tag-1 || 
				...
				if ((infos->host.file_info.flv.nb_video_tag >= 256) && (data_per_vtag == 0))cpt_video_tag=infos->host.file_info.flv.nb_video_tag;}
			else 
...
int extract_eoc(info_s * infos)
{
   ...
   if(infos->host.file_info.flv.nb_video_tag<256){
		...
		if (data_per_vtag == 0)
			k = infos->host.file_info.flv.nb_video_tag-2;
	...
	do {
		...
		else
		{
			...
			while(search_tag && fread(&tag_type, sizeof(uint8_t), 1, infos->host.host))
			{
