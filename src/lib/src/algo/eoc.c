    ...
    // à mettre à la fin
    	...
    	do{
    		...
    		if (byte_cpy!=9){
            	...
            	if (cpt_video_tag==infos->host.file_info.flv.nb_video_tag-1){data_size+=reste+data_per_vtag;} //cas particulier dernier tag
            	else {data_size+=reste+data_per_vtag;}
    			...
    			uint32_t limit;
    			if (cpt_video_tag==infos->host.file_info.flv.nb_video_tag-1){limit=data_per_vtag+reste;}
    			else{cpt_video_tag=data_per_vtag;}
    			...
    			if (cpt_video_tag==infos->host.file_info.flv.nb_video_tag-1){prev_tag_size+=reste+data_per_vtag;} //cas particulier dernier tag
            	else {prev_tag_size+=reste+data_per_vtag;}	
